...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 1300, 725)   #1600,1125 OG size
        # Create save action
        ...
        def create_cross_section_tab(self, Name):
            ...
            table.setFixedSize(700, 600)  #1100,600 OG size
    ...
    def clear_plots(self):
        ...
        for ax in axes:
            figure.delaxes(ax)
        figure.clear()
        canvas.setMinimumSize(math.ceil(10 * canvas.physicalDpiY()), int(10 * canvas.physicalDpiY()))
        canvas.setMaximumSize(math.ceil(10 * canvas.physicalDpiY()), int(10 * canvas.physicalDpiY()))
        scroll_area.setWidgetResizable(True)
        scroll_area.setWidget(canvas)
        canvas.draw()
    ...
    def plot_wEnergyResiduals(self):
        ...
        if len(excited_state_list) == 0:
            ...
            ax2.errorbar(angles, exp_energies[0], exp_energies_err[0], color='green', fmt='^', ecolor='#CEB888', markersize=4, capsize=2.0)
            ax2.hlines(excited_state_list[0], 15,60,colors='b', label='NNDC Energy')
            ax2.grid(True)
            ax2.set_xlabel(r'Lab Angle [$\Theta_{lab}$]')
            ax.set_xticks([])
            ax.minorticks_on()
            ax.set_title(f"{excited_state_list[0]} keV")
            ...
            canvas.setMinimumSize(math.ceil(total_height * canvas.physicalDpiY()), int(total_height * canvas.physicalDpiY()))
            canvas.setMaximumSize(math.ceil(total_height * canvas.physicalDpiY()), int(total_height * canvas.physicalDpiY()))
            scroll_area.setWidgetResizable(True)
            scroll_area.setWidget(canvas)
            counter = 1
        elif len(excited_state_list) == 2:
            fig_height = 5
            fig_width = 5
            figure.set_size_inches(fig_width, fig_height)
            figure.subplots_adjust(hspace=0.375, left=0.125, right = 0.625, top=0.920, bottom=0.080)
            total_height = 2 * fig_height + 0.375
            ...
            ax.errorbar(angles, cross_sections_list[1], yerr=error_list[1], color='black', fmt='x', ecolor='red', capsize=2.0, label='Data')
            ax.set_ylabel(r'Cross-Section [$\frac{mb}{sr}$]')
            ax.set_title(f"{excited_state_list[1]} keV")
            ax.set_yscale("log")
            ax.legend()

            ax2.errorbar(angles, exp_energies[1], exp_energies_err[1], color='green', fmt='^', ecolor='#CEB888', markersize=4, capsize=2.0)
            ax2.hlines(excited_state_list[1], 15,60,colors='b', label='NNDC Energy')
            ax2.grid(True)
            ax2.set_xlabel(r'Lab Angle [$\Theta_{lab}$]')
            ax.set_xticks([])
            ax.minorticks_on()
            ...
            if len(excited_state_list) > 9:
                ...
                spacing = 1 + 0.25 * plot_count  # Set spacing between subplots in inches
                total_height = total_height = subplot_height * plot_count + spacing  #(plot_count - 1)  # Calculate the total required height 
                figure.set_size_inches(5, subplot_height)  # Update the figure height
                counter = 0
                for i in range(len(excited_state_list)):
                    ...
                    for val in cross_sections_list[i]:
                        ...
                        if val > maxy:
                            maxy = 10
                    ax.set_ylim(miny, maxy)
                figure.subplots_adjust(hspace=0.310, left=0.196/plot_count, right=(1.355348*plot_count**-0.956521), top=0.980, bottom=(-0.01*plot_count + 0.11))
                canvas.setMinimumSize(math.ceil(total_height * canvas.physicalDpiY()), int(total_height * canvas.physicalDpiY()))
                canvas.setMaximumSize(math.ceil(total_height * canvas.physicalDpiY()), int(total_height * canvas.physicalDpiY()))
                scroll_area.setWidgetResizable(True)
                scroll_area.setWidget(canvas)

        text_display.append(f"{counter} energy-residual subplot(s) generated!")
        canvas.draw()
    ...
    def plot(self):
        ...
        text_display.append(f"{counter} cross-section subplot(s) created!")
        canvas.draw()
